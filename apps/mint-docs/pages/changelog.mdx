...
## Version 2.1 - Serverless Deployment Ready

### 🐛 Build Fixes
- **TypeScript Configuration**: Fixed tsconfig.json include paths for `api/` directory
- **Error Handling**: Improved TypeScript error handling for unknown error types
- **Build Process**: Successfully compiles TypeScript to JavaScript for deployment
...
- **Status**: Canvas-dependent features not available in serverless environments
- **Cause**: @napi-rs/canvas requires native dependencies (cairo, pixman)
- **Impact**: Text-to-images and PDF-to-images gracefully fail in serverless
- **Plan**: Features work in local environments with proper error handling

#### PDF to Image Conversions (Local Only)
- **Status**: Worker configuration issues persist in local environments
- **Cause**: pdfjs-dist library workerSrc type validation
- **Impact**: 2 out of 65 conversions fail locally (work in serverless with graceful degradation)
